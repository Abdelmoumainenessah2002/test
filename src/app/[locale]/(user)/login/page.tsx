...
const LoginPage = () => {
  ...
  return (
    <div className={`min-h-screen flex ${isRTL ? "flex-row-reverse" : "flex-row"}`}>
      {/* Right Section - Social Login with Background */}
      <div className="hidden lg:flex lg:w-1/2 relative">
        ...
        <div className="relative z-10 flex flex-col items-center w-full px-12 mt-20 text-white">
          <div className="max-w-md text-center">
            ...
            <h1 className="text-4xl font-bold mb-6">{t("welcomeBack")}</h1>
            <p className="text-xl mb-8 text-white text-opacity-90">{t("signInToContinue")}</p>
            ...
            <div className="mt-8 text-center">
              <p className="text-white text-opacity-80 text-sm">{t("secureAuthentication")}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white dark:bg-gray-800 bg-opacity-90 dark:bg-opacity-90 backdrop-blur-sm shadow-2xl border-0 rounded-2xl p-8">
            <div className="text-center pb-6">
              ...
              <p className="text-gray-600 dark:text-gray-300">{t("enterCredentials")}</p>
            </div>
            ...
            <div className="lg:hidden mt-6">
              ...
              <div className="space-y-3">
                ...
                <button className={`w-full bg-[#1877F2] hover:bg-[#166FE5] border border-[#1877F2] text-white py-3 rounded-lg font-normal transition-all duration-200 flex items-center justify-center dark:hover:bg-[#166FE5] whitespace-nowrap ${isRTL ? 'text-sm' : '' }`}>
                  <svg className={`w-5 h-5 ${isRTL ? "ml-2" : "mr-2"}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                  {t("continueWithFacebook")}
                </button>
                <button className={`w-full bg-[#24292F] hover:bg-[#1C2128] border border-[#24292F] text-white py-3 px-4 rounded-lg font-medium transition-all duration-200 flex items-center justify-center dark:border-white dark:hover:bg-gray-700 ${isRTL ? 'text-xs' : 'text-sm'} whitespace-nowrap`}>
                  <svg className={`w-5 h-5 ${isRTL ? "ml-2" : "mr-2"} flex-shrink-0`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.30.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                  </svg>
                  {t("continueWithGithub")}
                </button>
